...
@Component
public class InputWidgetJumpRule implements JumpRule {
    ...
    public boolean canJump(JumpRuleVO rule, UserFormAnswerVO answer) {
        ...
        for (JumpRuleVO.ExpressionVO expression : expressions) {
            ...
            if (expression.getExpression().equals("contains")) {
                s = String.format("'%s'.%s('%s')", answer.getValue(), expression.getExpression(), expression.getValue());
        ...
        System.out.println("input widget jump rule spel: " + spel);
